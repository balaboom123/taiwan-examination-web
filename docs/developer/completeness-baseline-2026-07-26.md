...
- 35 registered providers and 149,415 normalized paper records.
...
- 0 event-level download gaps, 288 normalization gaps, 8 normalized-but-not-published events, and 275 explicit publication-policy exclusions after the HCE, MOEX, New Taipei, and TCTE targeted repairs. The policy-aware provider-to-site check finds 2,418 expected and 2,418 actual logical site IDs, with zero missing or extra IDs.
...
| python3 -m app audit-catalog | **Pass**: 149,415 records; 2,989 review records; 653 queue entries; 667 mixed legacy groups requiring split | Strict mode passes because all review records have event-specific isolation and no review record is unapproved. It is an identity-safety result, not an archive-completeness result. |
| python3 -m app history-audit | Policy-aware non-strict **pass**; strict **fails** on 288 normalization gaps and 8 normalized-not-published events; 275 events are explicitly excluded by publication policy; download and parser gaps are both 0 | This is the most direct current provider-state/publication gap signal. |
...
| `wdasec_skill` | `https://owinform.wdasec.gov.tw/ExamNet/owInform/PastQuestions.aspx` | Workforce Development Agency skills-evaluation past questions | Raw sessions 2001–2026; usable paper records currently 2025–2026 | raw 2001–2026; papers 2025–2026, 2 buckets; 143 → 677 | **Partial / P0 / storage blocker** | ASP.NET WebForms/ViewState/postbacks; three administrations per year. A read-only fetch of official session 202411040001 returned 207 papers; all six 2024 sessions produced 453 papers and 422 mirrored payloads (~415 MB) in a temporary run. The 134 older raw sessions therefore represent stale accessible coverage, but expanding them requires a multi-gigabyte storage/release decision. |
...
2. Resolve the 288 normalization-gap events and 8 normalized-not-published Hakka events; separately disposition the WDASEC historical payload cost and oversized Hakka audio before treating them as releasable coverage. The 275 policy-excluded events must remain evidence-backed.
...
4. Reconcile the 667 mixed legacy groups requiring split and document the current physical/logical distinction: 2,421 physical assets, 2,418 logical IDs, with the policy-aware expected logical set equal to the site set.
...
- **Integrity gate:** The strict event-level audit still fails on 288 normalization gaps and 8 normalized-not-published Hakka events; download and parser gaps are 0, and 275 events are explicitly policy-excluded. Which historical items may be explicitly blocked/excluded, and which must be repaired before deployment is allowed?
