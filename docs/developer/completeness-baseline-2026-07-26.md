...
- 35 registered providers and 186,616 normalized paper records.
- 3,137 physical site/release bundle assets, represented as 3,134 logical frontend rows, across 11 release shards after the scoped WDASEC 2021–2024 and MOEX worker-promotion refreshes.
...
- 0 event-level download gaps, 129 normalization gaps, 8 normalized-but-not-published events, and 326 explicit publication-policy exclusions after the HCE, MOEX historical, MOEX worker-promotion, New Taipei, TCTE, and WDASEC 2021–2024 targeted repairs. The policy-aware provider-to-site check finds 3,134 expected and 3,134 actual logical site IDs, with zero missing or extra IDs.
...
- A read-only WDASEC adapter fetch of official session 202411040001 returned 207 paper rows. The normal pipeline then refreshed all six 2024 sessions into current provider state: 453 normalized papers, 422 valid mirrored payloads, zero failures, and approximately 400 MB. Scoped local publication added the eligible multi-year groups to the default site. Follow-up official probes found six 2021 sessions with 443 paper URLs, five 2022 sessions with 456 paper URLs, and five 2023 sessions with 447 paper URLs, all with zero discovery/detail errors; the targeted pipeline mirrored and normalized all 1,799 papers across 2021–2024 with zero sync failures. All five 2022 and 2023 sessions, all six 2024 sessions, and four 2021 sessions are published complete; two 2021 events remain explicitly excluded by site policy. The remaining 112 raw WDASEC events through 2020 are normalization gaps and require a separate storage/release decision.
...
| python3 scripts/validate_publication.py | **Pass**: 3,137 site bundles, 3,134 frontend bundles, 3,137 release assets, 10 schemas; expected and actual logical site IDs both 3,134 | Generated publication shapes and provider-derived public eligibility agree. This is not official-source completeness, remote-asset verification, or proof that oversized Hakka audio is releasable. |
| python3 -m app plan-release | **Pass**: 3,137 physical bundles across 11 release shards | Local release capacity is within the 900-per-shard target and 1,000-per-shard hard limit. It does not prove remote release assets exist. |
| python3 -m app audit-catalog | **Pass**: 186,616 records; 4,283 review records; 631 queue entries; 770 mixed legacy groups requiring split; 0 stale and 0 missing queue keys | Strict mode passes because all review records have event-specific isolation, no review record is unapproved, and generated queue keys agree with current papers. It is an identity-safety result, not an archive-completeness result. |
| python3 -m app history-audit | Policy-aware non-strict **pass**; strict **fails** on 129 normalization gaps and 8 normalized-not-published events; 326 events are explicitly excluded by publication policy; download and parser gaps are both 0 | This is the most direct current provider-state/publication gap signal. |
...
| `wdasec_skill` | `https://owinform.wdasec.gov.tw/ExamNet/owInform/PastQuestions.aspx` | Workforce Development Agency skills-evaluation past questions | Raw sessions 2001–2026; usable paper records 2021–2026 after the 2026-07-27 refresh | raw 2001–2026; papers 2021–2026, 6 buckets; 143 → 2,476 | **Partial / P0 / storage blocker** | ASP.NET WebForms/ViewState/postbacks; three administrations per year. Official 2021–2024 refreshes produced 1,799 papers (443, 456, 447, and 453) with zero sync failures; 27 event records are published complete and four are explicitly excluded by site policy, covering three single-year bundle IDs. Eligible multi-year groups now appear in the local default site. The remaining 112 older raw events through 2020 are normalization gaps. |
...
- Publication validation now reports 3,137 site assets, 3,134 frontend rows, and 3,137 release assets; the provider-derived expected logical set and site logical set are both 3,134 with zero difference. The MOEX worker-promotion cycle contributed 14 physical/frontend/release assets and changed no non-MOEX bundle; the subsequent WDASEC 2023, 2022, and 2021 cycles contributed 17, 14, and 8 physical/frontend/release assets, respectively.
...
2. Resolve the 129 normalization-gap events and 8 normalized-not-published Hakka events; separately disposition the 112 older WDASEC events, its historical payload cost, and oversized Hakka audio before treating them as releasable coverage. The 326 policy-excluded events must remain evidence-backed.
...
4. Reconcile the 770 mixed legacy groups requiring split and document the current physical/logical distinction: 3,137 physical assets, 3,134 logical IDs, with the policy-aware expected logical set equal to the site set.
...
- **Integrity gate:** The strict event-level audit still fails on 129 normalization gaps and 8 normalized-not-published Hakka events; download and parser gaps are 0, and 326 events are explicitly policy-excluded. Which historical items may be explicitly blocked/excluded, including the 11 MOEX source/file blockers and 112 older WDASEC events, and which must be repaired before deployment is allowed?
