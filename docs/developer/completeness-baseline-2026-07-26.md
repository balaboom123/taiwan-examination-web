...
- 0 event-level download gaps, 18 normalization gaps, 8 normalized-but-not-published events, and 330 explicit publication-policy exclusions after the HCE, MOEX historical, MOEX worker-promotion, New Taipei, TCTE, and WDASEC 2001–2024 targeted repairs. The policy-aware provider-to-site check finds 3,272 expected and 3,272 actual logical site IDs, with zero missing or extra IDs.
...
- The WDASEC 2002 refresh added 284 normalized papers and changed no aggregate local projection: it remains 3,273 physical assets and 3,270 logical frontend rows; seven of nine events are complete, one is explicitly excluded, and one official zero-paper event remains a normalization gap; it did not upload or deploy any release.
...
2. Resolve the 18 normalization-gap events and 8 normalized-not-published Hakka events; separately disposition the one official AD 2002 zero-paper WDASEC event, its historical payload cost, and oversized Hakka audio before treating them as releasable coverage. The 330 policy-excluded events must remain evidence-backed.
...
- **Integrity gate:** The strict event-level audit still fails on 18 normalization gaps and 8 normalized-not-published Hakka events; download and parser gaps are 0, and 330 events are explicitly policy-excluded. Which historical items may be explicitly blocked/excluded, including the 11 MOEX source/file blockers and the official AD 2002 zero-paper WDASEC event, and which must be repaired before deployment is allowed?
