...
- 35 registered providers and 194,309 normalized paper records.
- 3,273 physical site/release bundle assets, represented as 3,270 logical frontend rows, across 11 release shards after the scoped WDASEC 2003–2024 and MOEX worker-promotion refreshes.
...
- 0 event-level download gaps, 32 normalization gaps, 8 normalized-but-not-published events, and 329 explicit publication-policy exclusions after the HCE, MOEX historical, MOEX worker-promotion, New Taipei, TCTE, and WDASEC 2003–2024 targeted repairs. The policy-aware provider-to-site check finds 3,270 expected and 3,270 actual logical site IDs, with zero missing or extra IDs.
...
| python3 -m app audit-catalog | **Pass**: 194,309 records; 4,283 review records; 631 queue entries; 770 mixed legacy groups requiring split; 0 stale and 0 missing queue keys | Strict mode passes because all review records have event-specific isolation, no review record is unapproved, and generated queue keys agree with current papers. It is an identity-safety result, not an archive-completeness result. |
| python3 -m app history-audit | Policy-aware non-strict **pass**; strict **fails** on 32 normalization gaps and 8 normalized-not-published events; 329 events are explicitly excluded by publication policy; download and parser gaps are both 0 | This is the most direct current provider-state/publication gap signal. |
...
| `wdasec_skill` | `https://owinform.wdasec.gov.tw/ExamNet/owInform/PastQuestions.aspx` | Workforce Development Agency skills-evaluation past questions | Raw sessions 2001–2026; usable paper records 2003–2026 after the 2026-07-28 refresh | raw 2001–2026; papers 2003–2026, 24 buckets; 143 → 10,169 | **Partial / P0 / storage blocker** | ASP.NET WebForms/ViewState/postbacks; three administrations per year. Official 2003–2024 refreshes produced 9,492 papers (322, 303, 319, 376, 582, 342, 342, 369, 400, 426, 499, 479, 498, 534, 519, 534, 412, 437, 443, 456, 447, and 453) with zero sync failures; 121 event records are published complete and seven are explicitly excluded by site policy, with 12 unpublished bundle IDs across those exclusions. Two of four 2003 events are complete and two are explicitly excluded; two of three 2004 events are explicitly excluded and one is complete; two of three 2005 events are complete and one is explicitly excluded; two of three 2006 events are complete and one is explicitly excluded; two of five 2007 events are complete and three are explicitly excluded; four of five 2008 events are complete and one is explicitly excluded; all six 2009 events are published complete; six of seven 2010 events are complete and one is explicitly excluded; three of six 2011 events are complete and three are explicitly excluded; five of seven 2012 events are complete and two are explicitly excluded; one 2015 event remains excluded. Eligible multi-year groups now appear in the local default site. The remaining 15 older raw events through 2002 are normalization gaps. |
...
2. Resolve the 32 normalization-gap events and 8 normalized-not-published Hakka events; separately disposition the 15 older WDASEC events, its historical payload cost, and oversized Hakka audio before treating them as releasable coverage. The 329 policy-excluded events must remain evidence-backed.
...
- **Integrity gate:** The strict event-level audit still fails on 32 normalization gaps and 8 normalized-not-published Hakka events; download and parser gaps are 0, and 329 events are explicitly policy-excluded. Which historical items may be explicitly blocked/excluded, including the 11 MOEX source/file blockers and 15 older WDASEC events, and which must be repaired before deployment is allowed?
