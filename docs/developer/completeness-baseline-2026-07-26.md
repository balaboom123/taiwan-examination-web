...
- 35 registered providers and 194,593 normalized paper records.
- 3,273 physical site/release bundle assets, represented as 3,270 logical frontend rows, across 11 release shards after the scoped WDASEC 2002–2024 and MOEX worker-promotion refreshes.
...
- 0 event-level download gaps, 24 normalization gaps, 8 normalized-but-not-published events, and 330 explicit publication-policy exclusions after the HCE, MOEX historical, MOEX worker-promotion, New Taipei, TCTE, and WDASEC 2002–2024 targeted repairs. The policy-aware provider-to-site check finds 3,270 expected and 3,270 actual logical site IDs, with zero missing or extra IDs.
...
| python3 scripts/validate_publication.py | **Pass**: 3,273 site bundles, 3,270 frontend bundles, 3,273 release assets, 10 schemas; expected and actual logical site IDs both 3,270 | Generated publication shapes and provider-derived public eligibility agree. This is not official-source completeness, remote-asset verification, or proof that oversized Hakka audio is releasable. |
| python3 -m app plan-release | **Pass**: 3,273 physical bundles across 11 release shards | Local release capacity is within the 900-per-shard target and 1,000-per-shard hard limit. It does not prove remote release assets exist. |
| python3 -m app audit-catalog | **Pass**: 194,593 records; 4,283 review records; 631 queue entries; 770 mixed legacy groups requiring split; 0 stale and 0 missing queue keys | Strict mode passes because all review records have event-specific isolation, no review record is unapproved, and generated queue keys agree with current papers. It is an identity-safety result, not an archive-completeness result. |
| python3 -m app history-audit | Policy-aware non-strict **pass**; strict **fails** on 24 normalization gaps and 8 normalized-not-published events; 330 events are explicitly excluded by publication policy; download and parser gaps are both 0 | This is the most direct current provider-state/publication gap signal. |
...
| `wdasec_skill` | `https://owinform.wdasec.gov.tw/ExamNet/owInform/PastQuestions.aspx` | Workforce Development Agency skills-evaluation past questions | Raw sessions 2001–2026; usable paper records 2002–2026 after the 2026-07-28 refresh | raw 2001–2026; papers 2002–2026, 25 buckets; 143 → 10,453 | **Partial / P0 / storage blocker** | ASP.NET WebForms/ViewState/postbacks; three administrations per year. Official 2002–2024 refreshes produced 9,776 papers (284, 322, 303, 319, 376, 582, 342, 342, 369, 400, 426, 499, 479, 498, 534, 519, 534, 412, 437, 443, 456, 447, and 453) with zero sync failures; 128 event records are published complete and eight are explicitly excluded by site policy, with 13 unpublished bundle IDs across those exclusions. Seven of nine 2002 events are complete, one is explicitly excluded, and one remains a zero-paper normalization gap; two of four 2003 events are complete and two are explicitly excluded; two of three 2004 events are explicitly excluded and one is complete; two of three 2005 events are complete and one is explicitly excluded; two of three 2006 events are complete and one is explicitly excluded; two of five 2007 events are complete and three are explicitly excluded; four of five 2008 events are complete and one is explicitly excluded; all six 2009 events are published complete; six of seven 2010 events are complete and one is explicitly excluded; three of six 2011 events are complete and three are explicitly excluded; five of seven 2012 events are complete and two are explicitly excluded; one 2015 event remains excluded. Eligible multi-year groups now appear in the local default site. The remaining 7 older raw events through 2001 are normalization gaps. |
...
- Publication validation now reports 3,273 site assets, 3,270 frontend rows, and 3,273 release assets; the provider-derived expected logical set and site logical set are both 3,270 with zero difference. The MOEX worker-promotion cycle contributed 14 physical/frontend/release assets and changed no non-MOEX bundle; the subsequent WDASEC 2023, 2022, 2021, 2020, 2019, 2015, 2014, 2013, 2012, 2011, 2010, 2009, 2008, 2007, 2006, and 2005 cycles contributed 17, 14, 8, 2, 1, 3, 1, 35, 14, 3, 47, 0, 0, 3, 25, and 1 physical/frontend/release assets, respectively; the 2018 and 2017 cycles added no new site identity, while the 2016 cycle contributed one new identity.
...
2. Resolve the 24 normalization-gap events and 8 normalized-not-published Hakka events; separately disposition the 7 older WDASEC events, its historical payload cost, and oversized Hakka audio before treating them as releasable coverage. The 330 policy-excluded events must remain evidence-backed.
...
- **Integrity gate:** The strict event-level audit still fails on 24 normalization gaps and 8 normalized-not-published Hakka events; download and parser gaps are 0, and 330 events are explicitly policy-excluded. Which historical items may be explicitly blocked/excluded, including the 11 MOEX source/file blockers and 7 older WDASEC events, and which must be repaired before deployment is allowed?
