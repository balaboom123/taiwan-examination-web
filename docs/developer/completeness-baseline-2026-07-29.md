...
- A reviewed machine-readable scope inventory now covers all 35 registered providers and 10 candidate/watch sources. Its validator matches exact local years, raw event counts, normalized record counts, and failure counts; MOEX now has a complete 35-year/870-code official discovery manifest and all 870 listed events are represented locally, New Taipei, Tainan, Taoyuan elementary, Taipei elementary, and the reviewed 2024–2025 Taipei junior-high map also have complete declared-scope snapshots, while the other 29 providers still lack current discovery snapshots.
...
| --- | --- | --- |
| Official discovery | Provider contracts and adapters in `app/providers/`; reviewed scope is machine-validated by `catalog/source-inventory.json` and `scripts/validate_source_inventory.py`; MOEX discovery/probe state is manifest-backed. | 29 providers have no equivalent current manifest. MOEX has 870/870 local event representation and five teacher-recruitment snapshots match declared local scope, but that does not resolve evidence-backed source blockers or prove repository-wide archive completeness. |
...
| Full Python tests | **535 passed, 72 subtests passed** in 19.52 s via `uv run pytest -q` | Python functional baseline is green. |
...
| `python3 scripts/validate_source_inventory.py` | **Pass**: 35 providers and 10 candidates; local state matches the reviewed inventory; 6 discovery manifests are present, 29 are missing, and 0 manifest events are unrepresented | This is a scope/state-drift gate, not proof of live source completeness. `--require-discovery-manifests` remains intentionally failing until the 29 provider snapshots are resolved. |
| `python3 scripts/validate_source_inventory.py --require-discovery-manifests` | **Exit 1**: complete discovery remains unresolved for 29 providers with missing manifests; MOEX has 870/870, New Taipei 4/4, Tainan 1/1, Taoyuan elementary 1/1, Taipei junior-high 2/2, and Taipei elementary 1/1 local event representation | This is the authoritative completeness blocker; it prevents a manifest-only completeness claim. |
...
- Should the 29 providers without manifests be required to produce current discovery snapshots before any completeness claim?
