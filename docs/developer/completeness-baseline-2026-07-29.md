...
| --- | --- | --- |
| Official discovery | Provider contracts and adapters in `app/providers/`; reviewed scope is machine-validated by `catalog/source-inventory.json` and `scripts/validate_source_inventory.py`; MOEX discovery/probe state is manifest-backed. | 32 providers have no equivalent current manifest. MOEX has 870/870 local event representation and the two current-year teacher snapshots match local state, but that does not resolve evidence-backed source blockers or prove repository-wide archive completeness. |
...
| Full Python tests | **534 passed, 72 subtests passed** in 19.12 s via `uv run pytest -q` | Python functional baseline is green. |
...
| `python3 scripts/validate_source_inventory.py` | **Pass**: 35 providers and 10 candidates; local state matches the reviewed inventory; 3 discovery manifests are present, 32 are missing, and 0 manifest events are unrepresented | This is a scope/state-drift gate, not proof of live source completeness. `--require-discovery-manifests` remains intentionally failing until the 32 provider snapshots are resolved. |
| `python3 scripts/validate_source_inventory.py --require-discovery-manifests` | **Exit 1**: complete discovery remains unresolved for 32 providers with missing manifests; MOEX has 870/870, New Taipei 4/4, and Tainan 1/1 local event representation | This is the authoritative completeness blocker; it prevents a manifest-only completeness claim. |
...
- Should the 32 providers without manifests be required to produce current discovery snapshots before any completeness claim?
