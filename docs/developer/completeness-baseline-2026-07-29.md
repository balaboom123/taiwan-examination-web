...
| --- | --- | --- |
| Official discovery | Provider contracts and adapters in `app/providers/`; reviewed scope is machine-validated by `catalog/source-inventory.json` and `scripts/validate_source_inventory.py`; MOEX discovery/probe state is manifest-backed. | 17 providers still lack a snapshot; Kaohsiung is instead evidence-blocked without a fabricated manifest. MOEX has 870/870 local event representation, six teacher-recruitment snapshots match declared local scope, teacher qualification has 23/23, RCPET CAP has 15/15, WDASEC has 145/145, TCTE TVE has 26/26, special admission has 14/14 plus an exact 216-asset check, CMU HCE has 6/6 plus an exact 30-asset check, NSYSU HCE has 5/5 byte-verified combined PDFs, and NTHU HCE has 5/5 event representation plus 24/24 byte-verified assets; source blockers remain and none of this proves repository-wide archive completeness. |
...
| NTHU HCE discovery checkpoint | This commit (`data: complete NTHU HCE discovery`); history is preserved, not rewritten |
| Latest fetched `origin/main` | `1970a70` (`chore: refresh Hakka cert provider data`, 2026-07-29) |
| Divergence from `origin/main` after the NTHU HCE checkpoint | 8 behind / 78 ahead |
| Divergence from tracked upstream after the NTHU HCE checkpoint | 0 behind / 73 ahead; all are unpushed |
| Tracked uncommitted files after the NTHU HCE checkpoint | None |
| Untracked user work after the NTHU HCE checkpoint | `PLAN.md` only; preserved and excluded from all checkpoints |
...
| HCE archive focused tests | **22 passed, 4 subtests passed** | Covers bounded/paginated archive discovery, fail-closed embedded NTHU pagination and growth-bound rejection, fragment deduplication, stable provider discovery URLs, crawl-delay enforcement, TLS-exception scoping, combined-PDF handling, subject parsing, and shared-client behavior across four HCE providers. |
| Full Python tests | **563 passed, 72 subtests passed** in 19.09 s via `uv run pytest -q` | Python functional baseline is green. |
...
| `python3 scripts/validate_source_inventory.py` | **Pass**: 35 providers and 10 candidates; local state matches the reviewed inventory; 17 discovery manifests are present, 17 are missing, 1 is evidence-blocked, and 0 manifest events are unrepresented | This is a scope/state-drift gate, not proof of live source completeness. `--require-discovery-manifests` remains intentionally failing until the 17 unresolved provider snapshots are resolved. |
| `python3 scripts/validate_source_inventory.py --require-discovery-manifests` | **Exit 1**: complete discovery remains unresolved for 17 providers with missing manifests: `post_recruit`, `hce_tcu`, `cpc_recruit`, `moea_recruit`, `taipower_recruit`, `taisugar_recruit`, `twc_recruit`, `sfi_cert`, `tabf_cert`, `tii_cert`, `gept_cert`, `jlpt_cert`, `tocfl_cert`, `hakka_cert`, `taigi_cert`, `tqc_cert`, and `ipas_cert`. NTHU now has 5/5 local event representation and 24/24 byte-verified assets; Kaohsiung remains an evidence-backed discovery blocker. | This is the authoritative completeness gate; it accepts neither manifest-only coverage claims nor fabricated snapshots for unavailable sources. |
...
| Python line coverage | **84.40% executed lines**: 10,897/12,911 across 90 imported non-initializer `app` module cover files (shared HCE client: 331/370, 89.46%; NTHU provider: 14/21, 66.67%), measured with standard-library `trace --count --missing` | No branch coverage or threshold is configured; this is a reproducible measurement, not a CI gate. |
...
2. Recheck TCTE, special admission, CMU HCE, NSYSU HCE, and NTHU HCE on their annual archive cadences; preserve TCTE's PDF-preferred duplicate-format rule and special admission's explicit 大學組 / 共同 boundary.
...
- Should the 17 unresolved providers be required to produce current discovery snapshots before any completeness claim, while evidence-backed blockers such as Kaohsiung remain acceptable terminal states?

## Safest branch/release strategy

1. Keep `agent/exam-coverage-and-mirror-dedup` as a local audit/reference branch. Do not rebase it in place; after the NTHU HCE checkpoint it is 8 behind and 78 ahead of fetched `origin/main`, with 73 local commits beyond its tracked upstream.
2. Review `854bfe3` as the coherent CEEC code/data/site checkpoint, `3788eb7` as its documentation follow-up, and the RCPET, WDASEC, TCTE, special-admission, CMU HCE, NSYSU HCE, and NTHU HCE discovery checkpoints separately. Pre-existing untracked `PLAN.md` remains excluded.
