...
| --- | --- | --- |
| Official discovery | Provider contracts and adapters in `app/providers/`; reviewed scope is machine-validated by `catalog/source-inventory.json` and `scripts/validate_source_inventory.py`; MOEX discovery/probe state is manifest-backed. | 28 providers have no equivalent current manifest. MOEX has 870/870 local event representation and six teacher-recruitment event snapshots match declared local scope, but Central Alliance remains file-blocked and none of this proves repository-wide archive completeness. |
...
| Full Python tests | **536 passed, 72 subtests passed** in 20.74 s via `uv run pytest -q` | Python functional baseline is green. |
...
| `python3 scripts/validate_source_inventory.py` | **Pass**: 35 providers and 10 candidates; local state matches the reviewed inventory; 7 discovery manifests are present, 28 are missing, and 0 manifest events are unrepresented | This is a scope/state-drift gate, not proof of live source completeness. `--require-discovery-manifests` remains intentionally failing until the 28 provider snapshots are resolved. |
| `python3 scripts/validate_source_inventory.py --require-discovery-manifests` | **Exit 1**: complete discovery remains unresolved for 28 providers with missing manifests; MOEX has 870/870, New Taipei 4/4, Tainan 1/1, Taoyuan elementary 1/1, Taipei junior-high 2/2, Taipei elementary 1/1, and blocked Central Alliance 3/3 local event representation | This is the authoritative completeness blocker; it prevents a manifest-only completeness claim. |
...
- Should the 28 providers without manifests be required to produce current discovery snapshots before any completeness claim?
