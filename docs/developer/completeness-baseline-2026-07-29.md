...
| --- | --- | --- |
| Official discovery | Provider contracts and adapters in `app/providers/`; reviewed scope is machine-validated by `catalog/source-inventory.json` and `scripts/validate_source_inventory.py`; MOEX discovery/probe state is manifest-backed. | 33 providers have no equivalent current manifest. All 870 events in MOEX’s current official manifest are represented locally, but that does not resolve its evidence-backed source blockers or prove repository-wide archive completeness. |
...
| Full Python tests | **533 passed, 72 subtests passed** in 20.26 s via `uv run pytest -q` | Python functional baseline is green. |
...
| `python3 scripts/validate_source_inventory.py` | **Pass**: 35 providers and 10 candidates; local state matches the reviewed inventory; 2 discovery manifests are present, 33 are missing, and 0 manifest events are unrepresented | This is a scope/state-drift gate, not proof of live source completeness. `--require-discovery-manifests` remains intentionally failing until the 33 provider snapshots are resolved. |
| `python3 scripts/validate_source_inventory.py --require-discovery-manifests` | **Exit 1**: complete discovery remains unresolved for 33 providers with missing manifests; MOEX has 870/870 and New Taipei has 4/4 local event representation | This is the authoritative completeness blocker; it prevents a manifest-only completeness claim. |
...
- Should the 33 providers without manifests be required to produce current discovery snapshots before any completeness claim?
