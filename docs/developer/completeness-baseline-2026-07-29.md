...
| Full Python tests | **531 passed, 72 subtests passed** in 20.72 s via `uv run pytest -q` | Python functional baseline is green. |
...
| `python3 scripts/validate_source_inventory.py` | **Pass**: 35 providers and 10 candidates; local state matches the reviewed inventory; 1 discovery manifest is present, 34 are missing, and 0 manifest events are unrepresented | This is a scope/state-drift gate, not proof of live source completeness. `--require-discovery-manifests` remains intentionally failing until the 34 provider snapshots are resolved. |
| `python3 scripts/validate_source_inventory.py --require-discovery-manifests` | **Exit 1**: complete discovery remains unresolved for 34 providers with missing manifests; MOEX has 870/870 local event representation | This is the authoritative completeness blocker; it prevents a manifest-only completeness claim. |
