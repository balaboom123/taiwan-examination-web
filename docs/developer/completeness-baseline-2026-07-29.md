...
- 35 providers are registered in the default site. Current retained state contains 1,468 raw event pages and 244,471 normalized paper records after recovering MOEX AD 1994, AD 1998, AD 2007, AD 2017, and AD 2019–2021 and newly listed AD 2026 events.
- The local default projection contains 3,627 physical site/release assets, 3,624 frontend logical rows, and 12 planned release shards.
...
- The strict history audit reports 1,068 published-complete events, 15 event-level blocked exceptions, 3 partially blocked events, 373 publication-policy exclusions, and 8 normalized-but-not-published Hakka events. It reports zero parser gaps, zero normalization gaps, zero unclassified failure rows, and zero orphaned coverage exceptions.
...
| --- | --- | --- |
| Official discovery | Provider contracts and adapters in `app/providers/`; reviewed scope is machine-validated by `catalog/source-inventory.json` and `scripts/validate_source_inventory.py`; MOEX discovery/probe state is manifest-backed. | 26 providers still lack a snapshot; Kaohsiung is instead evidence-blocked without a fabricated manifest. MOEX has 870/870 local event representation, six teacher-recruitment snapshots match declared local scope, and teacher qualification has 23/23; source blockers remain and none of this proves repository-wide archive completeness. |
...
| Full Python tests | **538 passed, 72 subtests passed** in 19.82 s via `uv run pytest -q` | Python functional baseline is green. |
| `python3 scripts/validate_publication.py` | **Pass**: 3,627 site bundles, 3,624 frontend bundles, 3,627 release assets, 18 JSON schema/catalog files | Generated publication and provider-derived public eligibility agree. This does not prove official-source completeness or remote-asset existence. |
| `python3 -m app plan-release` | **Pass**: 3,627 bundles across 12 shards | Local shard capacity is within the 900 safety target; it does not verify remote releases. |
| `python3 scripts/validate_source_inventory.py` | **Pass**: 35 providers and 10 candidates; local state matches the reviewed inventory; 8 discovery manifests are present, 26 are missing, 1 is evidence-blocked, and 0 manifest events are unrepresented | This is a scope/state-drift gate, not proof of live source completeness. `--require-discovery-manifests` remains intentionally failing until the 26 unresolved provider snapshots are resolved. |
| `python3 scripts/validate_source_inventory.py --require-discovery-manifests` | **Exit 1**: complete discovery remains unresolved for 26 providers with missing manifests; MOEX has 870/870, teacher qualification 23/23, New Taipei 4/4, Tainan 1/1, Taoyuan elementary 1/1, Taipei junior-high 2/2, Taipei elementary 1/1, and blocked Central Alliance 3/3 local event representation; Kaohsiung is resolved as an evidence-backed discovery blocker | This is the authoritative completeness gate; it accepts neither manifest-only coverage claims nor fabricated snapshots for unavailable sources. |
...
| `python3 -m app audit-catalog --strict` | **Pass**: 244,471 records; 5,033 review records; 772 queue entries; 816 mixed legacy groups; 0 stale/missing queue keys; 0 unapproved review records | Identity/review state is fail-safe, not a claim that every official source is covered. |
...
- Should the 26 unresolved providers be required to produce current discovery snapshots before any completeness claim, while evidence-backed blockers such as Kaohsiung remain acceptable terminal states?
