...
| --- | --- | --- |
| Official discovery | Provider contracts and adapters in `app/providers/`; reviewed scope is machine-validated by `catalog/source-inventory.json` and `scripts/validate_source_inventory.py`; MOEX discovery/probe state is manifest-backed. | 27 providers still lack a snapshot; Kaohsiung is instead evidence-blocked without a fabricated manifest. MOEX has 870/870 local event representation and six teacher-recruitment event snapshots match declared local scope, but Central Alliance remains file-blocked and none of this proves repository-wide archive completeness. |
...
| Full Python tests | **537 passed, 72 subtests passed** in 20.11 s via `uv run pytest -q` | Python functional baseline is green. |
...
| `python3 scripts/validate_source_inventory.py` | **Pass**: 35 providers and 10 candidates; local state matches the reviewed inventory; 7 discovery manifests are present, 27 are missing, 1 is evidence-blocked, and 0 manifest events are unrepresented | This is a scope/state-drift gate, not proof of live source completeness. `--require-discovery-manifests` remains intentionally failing until the 27 unresolved provider snapshots are resolved. |
| `python3 scripts/validate_source_inventory.py --require-discovery-manifests` | **Exit 1**: complete discovery remains unresolved for 27 providers with missing manifests; MOEX has 870/870, New Taipei 4/4, Tainan 1/1, Taoyuan elementary 1/1, Taipei junior-high 2/2, Taipei elementary 1/1, and blocked Central Alliance 3/3 local event representation; Kaohsiung is resolved as an evidence-backed discovery blocker | This is the authoritative completeness gate; it accepts neither manifest-only coverage claims nor fabricated snapshots for unavailable sources. |
...
- Should the 27 unresolved providers be required to produce current discovery snapshots before any completeness claim, while evidence-backed blockers such as Kaohsiung remain acceptable terminal states?
