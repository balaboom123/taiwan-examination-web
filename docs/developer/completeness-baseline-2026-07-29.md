...
| --- | --- | --- |
| Official discovery | Provider contracts and adapters in `app/providers/`; reviewed scope is machine-validated by `catalog/source-inventory.json` and `scripts/validate_source_inventory.py`; MOEX discovery/probe state is manifest-backed. | 20 providers still lack a snapshot; Kaohsiung is instead evidence-blocked without a fabricated manifest. MOEX has 870/870 local event representation, six teacher-recruitment snapshots match declared local scope, teacher qualification has 23/23, RCPET CAP has 15/15, WDASEC has 145/145, TCTE TVE has 26/26, and special admission has 14/14 plus a separate exact 216-asset check; source blockers remain and none of this proves repository-wide archive completeness. |
...
| Special-admission discovery checkpoint | This commit (`data: complete special-admission discovery`); history is preserved, not rewritten |
| Latest fetched `origin/main` | `1970a70` (`chore: refresh Hakka cert provider data`, 2026-07-29) |
| Divergence from `origin/main` after the special-admission checkpoint | 8 behind / 75 ahead |
| Divergence from tracked upstream after the special-admission checkpoint | 0 behind / 70 ahead; all are unpushed |
| Tracked uncommitted files after the special-admission checkpoint | None |
| Untracked user work after the special-admission checkpoint | `PLAN.md` only; preserved and excluded from all checkpoints |
...
| Full Python tests | **555 passed, 72 subtests passed** in 19.42 s via `uv run pytest -q` | Python functional baseline is green. |
...
| `python3 scripts/validate_source_inventory.py` | **Pass**: 35 providers and 10 candidates; local state matches the reviewed inventory; 14 discovery manifests are present, 20 are missing, 1 is evidence-blocked, and 0 manifest events are unrepresented | This is a scope/state-drift gate, not proof of live source completeness. `--require-discovery-manifests` remains intentionally failing until the 20 unresolved provider snapshots are resolved. |
| `python3 scripts/validate_source_inventory.py --require-discovery-manifests` | **Exit 1**: complete discovery remains unresolved for 20 providers with missing manifests; GSAT has 179/179, AST 31/31, RCPET CAP 15/15, WDASEC 145/145, TCTE TVE 26/26, and special admission 14/14 local event representation in addition to the earlier complete snapshots; Kaohsiung remains an evidence-backed discovery blocker | This is the authoritative completeness gate; it accepts neither manifest-only coverage claims nor fabricated snapshots for unavailable sources. |
...
| Python line coverage | **84.36% executed lines**: 10,850/12,861 across 90 imported non-initializer `app` module cover files (special-admission client: 157/179, 87.71%), measured with standard-library `trace --count --missing` | No branch coverage or threshold is configured; this is a reproducible measurement, not a CI gate. |
...
### Hakka, WDASEC, TCTE, and special admission
...
2. Recheck TCTE and special admission on their annual archive cadences; preserve TCTE's PDF-preferred duplicate-format rule and special admission's explicit 大學組 / 共同 boundary.
...
- Should the 20 unresolved providers be required to produce current discovery snapshots before any completeness claim, while evidence-backed blockers such as Kaohsiung remain acceptable terminal states?

## Safest branch/release strategy

1. Keep `agent/exam-coverage-and-mirror-dedup` as a local audit/reference branch. Do not rebase it in place; after the special-admission checkpoint it is 8 behind and 75 ahead of fetched `origin/main`, with 70 local commits beyond its tracked upstream.
2. Review `854bfe3` as the coherent CEEC code/data/site checkpoint, `3788eb7` as its documentation follow-up, and the RCPET, WDASEC, TCTE, and special-admission discovery checkpoints separately. Pre-existing untracked `PLAN.md` remains excluded.
