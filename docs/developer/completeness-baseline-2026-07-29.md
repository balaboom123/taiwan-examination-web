...
- 35 providers are registered in the default site. Current retained state contains 1,556 raw event pages and 244,997 normalized paper records after recovering GSAT AD 1994–2010 and refreshing current AST materials in addition to the earlier MOEX recoveries.
...
- The strict history audit reports 1,160 published-complete events, 13 event-level blocked exceptions, 3 partially blocked events, 371 publication-policy exclusions, and 8 normalized-but-not-published Hakka events. It reports zero parser gaps, zero normalization gaps, zero unclassified failure rows, and zero orphaned coverage exceptions.
...
| --- | --- | --- |
| Official discovery | Provider contracts and adapters in `app/providers/`; reviewed scope is machine-validated by `catalog/source-inventory.json` and `scripts/validate_source_inventory.py`; MOEX discovery/probe state is manifest-backed. | 16 providers still lack a snapshot; Kaohsiung is instead evidence-blocked without a fabricated manifest. MOEX has 870/870 local event representation, six teacher-recruitment snapshots match declared local scope, teacher qualification has 23/23, RCPET CAP has 15/15, WDASEC has 145/145, TCTE TVE has 26/26, special admission has 14/14 plus an exact 216-asset check, CMU HCE has 6/6 plus an exact 30-asset check, NSYSU HCE has 5/5 byte-verified combined PDFs, NTHU HCE has 5/5 event representation plus 24/24 byte-verified assets, and TCU HCE has 1/1 current event representation plus 8/8 byte-verified assets and source-removal evidence for 2025; source blockers remain and none of this proves repository-wide archive completeness. |
...
| TCU HCE discovery checkpoint | This commit (`data: reconcile TCU HCE discovery`); history is preserved, not rewritten |
| Latest fetched `origin/main` | `1970a70` (`chore: refresh Hakka cert provider data`, 2026-07-29) |
| Divergence from `origin/main` after the TCU HCE checkpoint | 8 behind / 79 ahead |
| Divergence from tracked upstream after the TCU HCE checkpoint | 0 behind / 74 ahead; all are unpushed |
| Tracked uncommitted files after the TCU HCE checkpoint | None |
| Untracked user work after the TCU HCE checkpoint | `PLAN.md` only; preserved and excluded from all checkpoints |
...
| Full Python tests | **565 passed, 72 subtests passed** in 19.50 s via `uv run pytest -q` | Python functional baseline is green. |
...
| `python3 scripts/validate_source_inventory.py` | **Pass**: 35 providers and 10 candidates; local state matches the reviewed inventory; 18 discovery manifests are present, 16 are missing, 1 is evidence-blocked, and 0 manifest events are unrepresented | This is a scope/state-drift gate, not proof of live source completeness. `--require-discovery-manifests` remains intentionally failing until the 16 unresolved provider snapshots are resolved. |
| `python3 scripts/validate_source_inventory.py --require-discovery-manifests` | **Exit 1**: complete discovery remains unresolved for 16 providers with missing manifests: `post_recruit`, `cpc_recruit`, `moea_recruit`, `taipower_recruit`, `taisugar_recruit`, `twc_recruit`, `sfi_cert`, `tabf_cert`, `tii_cert`, `gept_cert`, `jlpt_cert`, `tocfl_cert`, `hakka_cert`, `taigi_cert`, `tqc_cert`, and `ipas_cert`. TCU now has 1/1 current event representation and 8/8 byte-verified assets, while known 2025 official URLs are retained as source-removal evidence; Kaohsiung remains an evidence-backed discovery blocker. | This is the authoritative completeness gate; it accepts neither manifest-only coverage claims nor fabricated snapshots for unavailable sources. |
| `python3 -m app history-audit --strict` | **Exit 1**: only the 8 Hakka `normalized_not_published` events remain unresolved; 13 blocked and 3 partial events are evidence-backed | The gate no longer hides the five current MOEX download failures and rejects coverage-exception conflicts/orphans. |
| `python3 -m app audit-catalog --strict` | **Pass**: 244,997 records; 5,033 review records; 772 queue entries; 816 mixed legacy groups; 0 stale/missing queue keys; 0 unapproved review records | Identity/review state is fail-safe, not a claim that every official source is covered. |
| Python line coverage | **84.41% executed lines**: 10,901/12,915 across 90 imported non-initializer `app` module cover files (shared HCE client: 331/370, 89.46%; TCU provider: 16/21, 76.19%), measured with standard-library `trace --count --missing` | No branch coverage or threshold is configured; this is a reproducible measurement, not a CI gate. |
...
2. Recheck TCTE, special admission, CMU HCE, NSYSU HCE, NTHU HCE, and TCU HCE on their annual archive cadences; preserve TCTE's PDF-preferred duplicate-format rule and special admission's explicit 大學組 / 共同 boundary.
...
- Should the 16 unresolved providers be required to produce current discovery snapshots before any completeness claim, while evidence-backed blockers such as Kaohsiung and TCU's removed historical material remain acceptable terminal states?

## Safest branch/release strategy

1. Keep `agent/exam-coverage-and-mirror-dedup` as a local audit/reference branch. Do not rebase it in place; after the TCU HCE checkpoint it is 8 behind and 79 ahead of fetched `origin/main`, with 74 local commits beyond its tracked upstream.
2. Review `854bfe3` as the coherent CEEC code/data/site checkpoint, `3788eb7` as its documentation follow-up, and the RCPET, WDASEC, TCTE, special-admission, CMU HCE, NSYSU HCE, NTHU HCE, and TCU HCE discovery checkpoints separately. Pre-existing untracked `PLAN.md` remains excluded.
