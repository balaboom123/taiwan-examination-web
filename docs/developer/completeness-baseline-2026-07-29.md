...
| --- | --- | --- |
| Official discovery | Provider contracts and adapters in `app/providers/`; reviewed scope is machine-validated by `catalog/source-inventory.json` and `scripts/validate_source_inventory.py`; MOEX discovery/probe state is manifest-backed. | 18 providers still lack a snapshot; Kaohsiung is instead evidence-blocked without a fabricated manifest. MOEX has 870/870 local event representation, six teacher-recruitment snapshots match declared local scope, teacher qualification has 23/23, RCPET CAP has 15/15, WDASEC has 145/145, TCTE TVE has 26/26, special admission has 14/14 plus an exact 216-asset check, CMU HCE has 6/6 plus an exact 30-asset check, and NSYSU HCE has 5/5 byte-verified combined PDFs; source blockers remain and none of this proves repository-wide archive completeness. |
...
| NSYSU HCE discovery checkpoint | This commit (`data: complete NSYSU HCE discovery`); history is preserved, not rewritten |
| Latest fetched `origin/main` | `1970a70` (`chore: refresh Hakka cert provider data`, 2026-07-29) |
| Divergence from `origin/main` after the NSYSU HCE checkpoint | 8 behind / 77 ahead |
| Divergence from tracked upstream after the NSYSU HCE checkpoint | 0 behind / 72 ahead; all are unpushed |
| Tracked uncommitted files after the NSYSU HCE checkpoint | None |
| Untracked user work after the NSYSU HCE checkpoint | `PLAN.md` only; preserved and excluded from all checkpoints |
...
| HCE archive focused tests | **19 passed, 4 subtests passed** | Covers bounded/paginated archive discovery, fragment deduplication, stable provider discovery URLs, crawl-delay enforcement, TLS-exception scoping, combined-PDF handling, subject parsing, and shared-client behavior across four HCE providers. |
| Full Python tests | **560 passed, 72 subtests passed** in 19.43 s via `uv run pytest -q` | Python functional baseline is green. |
...
| `python3 scripts/validate_source_inventory.py` | **Pass**: 35 providers and 10 candidates; local state matches the reviewed inventory; 16 discovery manifests are present, 18 are missing, 1 is evidence-blocked, and 0 manifest events are unrepresented | This is a scope/state-drift gate, not proof of live source completeness. `--require-discovery-manifests` remains intentionally failing until the 18 unresolved provider snapshots are resolved. |
| `python3 scripts/validate_source_inventory.py --require-discovery-manifests` | **Exit 1**: complete discovery remains unresolved for 18 providers with missing manifests; GSAT has 179/179, AST 31/31, RCPET CAP 15/15, WDASEC 145/145, TCTE TVE 26/26, special admission 14/14, CMU HCE 6/6, and NSYSU HCE 5/5 local event representation in addition to the earlier complete snapshots; Kaohsiung remains an evidence-backed discovery blocker | This is the authoritative completeness gate; it accepts neither manifest-only coverage claims nor fabricated snapshots for unavailable sources. |
...
| Python line coverage | **84.41% executed lines**: 10,885/12,895 across 90 imported non-initializer `app` module cover files (shared HCE client: 321/358, 89.66%; NSYSU provider: 16/21, 76.19%), measured with standard-library `trace --count --missing` | No branch coverage or threshold is configured; this is a reproducible measurement, not a CI gate. |
...
2. Recheck TCTE, special admission, CMU HCE, and NSYSU HCE on their annual archive cadences; preserve TCTE's PDF-preferred duplicate-format rule and special admission's explicit 大學組 / 共同 boundary.
...
- Should the 18 unresolved providers be required to produce current discovery snapshots before any completeness claim, while evidence-backed blockers such as Kaohsiung remain acceptable terminal states?

## Safest branch/release strategy

1. Keep `agent/exam-coverage-and-mirror-dedup` as a local audit/reference branch. Do not rebase it in place; after the NSYSU HCE checkpoint it is 8 behind and 77 ahead of fetched `origin/main`, with 72 local commits beyond its tracked upstream.
2. Review `854bfe3` as the coherent CEEC code/data/site checkpoint, `3788eb7` as its documentation follow-up, and the RCPET, WDASEC, TCTE, special-admission, CMU HCE, and NSYSU HCE discovery checkpoints separately. Pre-existing untracked `PLAN.md` remains excluded.
