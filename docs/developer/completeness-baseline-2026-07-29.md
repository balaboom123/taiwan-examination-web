...
| --- | --- | --- |
| Official discovery | Provider contracts and adapters in `app/providers/`; reviewed scope is machine-validated by `catalog/source-inventory.json` and `scripts/validate_source_inventory.py`; MOEX discovery/probe state is manifest-backed. | 24 providers still lack a snapshot; Kaohsiung is instead evidence-blocked without a fabricated manifest. MOEX has 870/870 local event representation, six teacher-recruitment snapshots match declared local scope, and teacher qualification has 23/23; source blockers remain and none of this proves repository-wide archive completeness. |
...
| Final report handoff | Documentation follow-up after the CEEC checkpoint; history is preserved, not rewritten |
| Latest fetched `origin/main` | `1970a70` (`chore: refresh Hakka cert provider data`, 2026-07-29) |
| Divergence from `origin/main` at final handoff | 8 behind / 71 ahead |
| Divergence from tracked upstream at final handoff | 0 behind / 66 ahead; all are unpushed |
...
| Full Python tests | **543 passed, 72 subtests passed** in 18.81 s via `uv run pytest -q` | Python functional baseline is green. |
...
| `python3 scripts/validate_source_inventory.py` | **Pass**: 35 providers and 10 candidates; local state matches the reviewed inventory; 10 discovery manifests are present, 24 are missing, 1 is evidence-blocked, and 0 manifest events are unrepresented | This is a scope/state-drift gate, not proof of live source completeness. `--require-discovery-manifests` remains intentionally failing until the 24 unresolved provider snapshots are resolved. |
| `python3 scripts/validate_source_inventory.py --require-discovery-manifests` | **Exit 1**: complete discovery remains unresolved for 24 providers with missing manifests; GSAT has 179/179 and AST 31/31 local event representation in addition to the earlier complete snapshots; Kaohsiung remains an evidence-backed discovery blocker | This is the authoritative completeness gate; it accepts neither manifest-only coverage claims nor fabricated snapshots for unavailable sources. |
...
| Python line coverage | **83.61% executed lines**: 10,573/12,645 across 118 traced `app` modules, measured with standard-library `trace` | No branch coverage or threshold is configured; this is a reproducible measurement, not a CI gate. |
...
- Should the 24 unresolved providers be required to produce current discovery snapshots before any completeness claim, while evidence-backed blockers such as Kaohsiung remain acceptable terminal states?

## Safest branch/release strategy

1. Keep `agent/exam-coverage-and-mirror-dedup` as a local audit/reference branch. Do not rebase it in place; after this documentation follow-up it is 8 behind and 71 ahead of fetched `origin/main`, with 66 local commits beyond its tracked upstream.
2. Review `854bfe3` as the coherent CEEC code/data/site checkpoint and its documentation-only follow-up separately. Pre-existing untracked `PLAN.md` remains excluded.
