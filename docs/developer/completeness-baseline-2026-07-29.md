...
- A reviewed machine-readable scope inventory now covers all 35 registered providers and 10 candidate/watch sources. Its validator matches exact local years, raw event counts, normalized record counts, and failure counts; MOEX now has a complete 35-year/870-code official discovery manifest and all 870 listed events are represented locally, New Taipei, Tainan, and Taoyuan elementary also have complete current-year snapshots, while the other 31 providers still lack current discovery snapshots.
...
| --- | --- | --- |
| Official discovery | Provider contracts and adapters in `app/providers/`; reviewed scope is machine-validated by `catalog/source-inventory.json` and `scripts/validate_source_inventory.py`; MOEX discovery/probe state is manifest-backed. | 31 providers have no equivalent current manifest. MOEX has 870/870 local event representation and the three current-year teacher snapshots match local state, but that does not resolve evidence-backed source blockers or prove repository-wide archive completeness. |
...
| Full Python tests | **535 passed, 72 subtests passed** in 19.76 s via `uv run pytest -q` | Python functional baseline is green. |
...
| `python3 scripts/validate_source_inventory.py` | **Pass**: 35 providers and 10 candidates; local state matches the reviewed inventory; 4 discovery manifests are present, 31 are missing, and 0 manifest events are unrepresented | This is a scope/state-drift gate, not proof of live source completeness. `--require-discovery-manifests` remains intentionally failing until the 31 provider snapshots are resolved. |
| `python3 scripts/validate_source_inventory.py --require-discovery-manifests` | **Exit 1**: complete discovery remains unresolved for 31 providers with missing manifests; MOEX has 870/870, New Taipei 4/4, Tainan 1/1, and Taoyuan elementary 1/1 local event representation | This is the authoritative completeness blocker; it prevents a manifest-only completeness claim. |
...
- Should the 31 providers without manifests be required to produce current discovery snapshots before any completeness claim?
