...
- legacy static HTML output stored under `site/`
...
| `app/publisher.py` | write generated JSON files and legacy `site/` output |
...
| `sync-full` | rebuild from the live source | full generated data, bundles, and site output |
| `build-bundles` | rebuild ZIP bundles from existing local state only | updated `bundles/`, `data/bundles.json`, `site/` |
| `build-site` | rebuild legacy `site/` output from generated data | `site/index.html` and site data |
...
There are two output surfaces today:

1. Legacy static site
   - built by `app.publisher.build_site`
   - stored under `site/`
   - committed by sync workflows

2. Modern frontend app
   - source code under `frontend/`
   - consumes generated bundle data at build and dev time
   - deploy workflow currently targets GitHub Pages

The modern frontend is the strategic UI path. The legacy `site/` output still exists and is regenerated by sync workflows, so it remains part of the supported current behavior.
...
7. `app/publisher.py` writes generated JSON files and legacy static site output.
