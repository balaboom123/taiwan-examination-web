...
- 35 registered providers and 194,835 normalized paper records.
- 3,275 physical site/release bundle assets, represented as 3,272 logical frontend rows, across 11 release shards after the scoped WDASEC 2001–2024 and MOEX worker-promotion refreshes.
...
- 0 event-level download gaps, 18 normalization gaps, 8 normalized-but-not-published events, and 330 explicit publication-policy exclusions after the HCE, MOEX historical, MOEX worker-promotion, New Taipei, TCTE, and WDASEC 2001–2024 targeted repairs. One of the 18 normalization gaps is now backed by a bounded official WDASEC AD 2002 empty-result capture; it is not a download failure. The policy-aware provider-to-site check finds 3,272 expected and 3,272 actual logical site IDs, with zero missing or extra IDs.
...
| Normalization and identity | app/normalizer.py emits NormalizedPaper records and review candidates. app/classification.py derives exam identity v2 dimensions and isolates unresolved evidence by event. | 4,283 records still have review confidence; 18 events have normalization gaps, including one verified official WDASEC no-data result. A passing review-isolation audit means records are safely separated, not semantically resolved. |
...
### Final live handoff snapshot (2026-07-28)

A final `git fetch --all --prune` refreshed `origin/main` to `8be0e4f` (`chore: refresh TABF cert provider data`, 2026-07-27). After the documentation commit that records this snapshot, the expected live divergence is `7 behind / 49 ahead` against `origin/main` and `0 behind / 44 ahead` against the tracked upstream `origin/agent/exam-coverage-and-mirror-dedup`; these local commits remain unpushed. The tracked worktree is clean. `PLAN.md` is the only untracked file (about 40 KB); there are no large untracked Git files. The ignored operational trees are approximately 396 MB under `data/`, 57 GB under `mirror/`, and 61 GB under `bundles/`; their size is not evidence of publication or remote release coverage. No push, release upload, deployment, history rewrite, or credential use occurred.

### Post-baseline bounded source/release investigation

- A read-only WDASEC adapter fetch of official session 202411040001 returned 207 paper rows. The normal pipeline then refreshed all six 2024 sessions into current provider state: 453 normalized papers, 422 valid mirrored payloads, zero failures, and approximately 400 MB. Scoped local publication added the eligible multi-year groups to the default site. Follow-up official probes found three 2005 sessions with 319 paper URLs, three 2006 sessions with 376 paper URLs, five 2007 sessions with 582 paper URLs, five 2008 sessions with 342 paper URLs, six 2009 sessions with 342 paper URLs, six 2011 sessions with 400 paper URLs, seven 2012 sessions with 426 paper URLs, six 2013 sessions with 499 paper URLs, six 2014 sessions with 479 paper URLs, six 2015 sessions with 498 paper URLs, six 2016 sessions with 534 paper URLs, six 2017 sessions with 519 paper URLs, six 2018 sessions with 534 paper URLs, six 2019 sessions with 412 paper URLs, six 2020 sessions with 437 paper URLs, six 2021 sessions with 443 paper URLs, five 2022 sessions with 456 paper URLs, and five 2023 sessions with 447 paper URLs, all with zero discovery/detail errors. A subsequent AD 2004 probe found three official sessions with 303 paper URLs and zero discovery/detail errors; the next AD 2003 probe found four official sessions with 322 paper URLs and zero discovery/detail errors; the next AD 2002 probe found nine official sessions with zero discovery/detail errors: eight exposed 284 paper URLs and one exposed no paper rows without a parser/download error; the AD 2001 probe found six official sessions with 242 paper URLs and zero discovery/detail errors; the targeted pipeline mirrored and normalized all 9,223 papers across AD 2001–2023 with zero sync failures. The two AD 2003 first-batch records were retained as separate official source events; their eligible identities converge under the existing canonical/publication rules. Five of six 2001 events are complete and one is explicitly excluded; the AD 2002 projection now has eight complete events and one zero-paper normalization gap after the previously policy-excluded first-batch event 201201050019 became eligible through the AD 2001 multi-year identity; two of four 2003 events are complete and two are explicitly excluded; two of three 2004 events are explicitly excluded and one is complete; two of three 2005 events are complete, with one explicitly excluded; two of three 2006 events are complete, with one explicitly excluded; two of five 2007 events are complete, with three explicitly excluded; four of five 2008 events are complete, with one explicitly excluded; all six 2009 events are published complete; three 2011 events are complete, with three explicitly excluded; all 2013–2014 sessions, all 2016–2021 sessions, all five 2022 and 2023 sessions, and all six 2024 sessions are published complete; five 2012 events are complete, with two explicitly excluded, and one 2015 event remains explicitly excluded. The remaining normalization gap is the official AD 2002 zero-paper event and requires a separate storage/release decision.
...
- The WDASEC 2007 refresh added 582 normalized papers and changed the aggregate local projection from 3,244 to 3,247 physical assets and from 3,241 to 3,244 logical frontend rows; it did not upload or deploy any release.
- The WDASEC 2006 refresh added 376 normalized papers and changed the aggregate local projection from 3,247 to 3,272 physical assets and from 3,244 to 3,269 logical frontend rows; it did not upload or deploy any release.
- The WDASEC 2005 refresh added 319 normalized papers and changed the aggregate local projection from 3,272 to 3,273 physical assets and from 3,269 to 3,270 logical frontend rows; it did not upload or deploy any release.
- The WDASEC 2004 refresh added 303 normalized papers and changed no aggregate local projection: it remains 3,273 physical assets and 3,270 logical frontend rows; the one complete event reused existing logical identities while two events are explicitly excluded by publication policy; it did not upload or deploy any release.
- The WDASEC 2003 refresh added 322 normalized papers and changed no aggregate local projection: it remains 3,273 physical assets and 3,270 logical frontend rows; two of four events are complete and two are explicitly excluded by publication policy; it did not upload or deploy any release.
- The WDASEC 2002 refresh added 284 normalized papers and changed no aggregate local projection: it remains 3,273 physical assets and 3,270 logical frontend rows; eight of nine events are complete and one official zero-paper event remains a normalization gap; it did not upload or deploy any release.
- The WDASEC 2001 refresh added 242 normalized papers and increased the aggregate local projection from 3,273 to 3,275 physical assets and from 3,270 to 3,272 logical frontend rows; five of six events are complete and one is explicitly excluded. It also made the AD 2002 first-batch identity eligible through the multi-year policy; it did not upload or deploy any release.
- The remaining WDASEC AD 2002 event `201309140001` is a verified official source-side empty result, not a failed download: the 2026-07-28 bounded capture reached listing page 9 row 10, matched the title `91年度全國檢定第1梯次學科試題暨答案`, received a 9,402-byte detail page with SHA-256 `66ea45bb8e00d0be4e780796adcce473e6515ce2498cbf6ee4cfba0a43ef6dc0`, parsed zero `gvFile` rows, and displayed `查無資料，請確定輸入資料並重新查詢`. The raw page is retained for traceability, but the current history audit has no generic source-empty exception ledger, so this event remains a visible normalization gap pending an approved blocked/excluded status policy; no paper row or mirror payload was fabricated.
...
| python3 scripts/validate_publication.py | **Pass**: 3,275 site bundles, 3,272 frontend bundles, 3,275 release assets, 10 schemas; expected and actual logical site IDs both 3,272 | Generated publication shapes and provider-derived public eligibility agree. This is not official-source completeness, remote-asset verification, or proof that oversized Hakka audio is releasable. |
| python3 -m app plan-release | **Pass**: 3,275 physical bundles across 11 release shards | Local release capacity is within the 900-per-shard target and 1,000-per-shard hard limit. It does not prove remote release assets exist. |
| python3 -m app audit-catalog | **Pass**: 194,835 records; 4,283 review records; 631 queue entries; 770 mixed legacy groups requiring split; 0 stale and 0 missing queue keys | Strict mode passes because all review records have event-specific isolation, no review record is unapproved, and generated queue keys agree with current papers. It is an identity-safety result, not an archive-completeness result. |
| python3 -m app history-audit | Policy-aware non-strict **pass**; strict **fails** on 18 normalization gaps and 8 normalized-not-published events; 330 events are explicitly excluded by publication policy; download and parser gaps are both 0 | This is the most direct current provider-state/publication gap signal. |
...
| `wdasec_skill` | `https://owinform.wdasec.gov.tw/ExamNet/owInform/PastQuestions.aspx` | Workforce Development Agency skills-evaluation past questions | Raw sessions 2001–2026; usable paper records 2001–2026 after the 2026-07-28 refresh | raw 2001–2026; papers 2001–2026, 26 buckets; 143 → 10,695 | **Partial / P0 / storage blocker** | ASP.NET WebForms/ViewState/postbacks; three administrations per year. Official 2001–2024 refreshes produced 10,018 papers (242, 284, 322, 303, 319, 376, 582, 342, 342, 369, 400, 426, 499, 479, 498, 534, 519, 534, 412, 437, 443, 456, 447, and 453) with zero sync failures; 134 event records are published complete and eight are explicitly excluded by site policy, with 12 unpublished bundle IDs across those exclusions. Eight of nine 2002 events are complete and one remains a zero-paper normalization gap; five of six 2001 events are complete and one is explicitly excluded; two of four 2003 events are complete and two are explicitly excluded; two of three 2004 events are explicitly excluded and one is complete; two of three 2005 events are complete and one is explicitly excluded; two of three 2006 events are complete and one is explicitly excluded; two of five 2007 events are complete and three are explicitly excluded; four of five 2008 events are complete and one is explicitly excluded; all six 2009 events are published complete; six of seven 2010 events are complete and one is explicitly excluded; three of six 2011 events are complete and three are explicitly excluded; five of seven 2012 events are complete and two are explicitly excluded; one 2015 event remains excluded. Eligible multi-year groups now appear in the local default site. The remaining raw event is `201309140001` (AD 2002), whose official detail page returned zero rows and `查無資料`; its captured detail hash is `66ea45bb8e00d0be4e780796adcce473e6515ce2498cbf6ee4cfba0a43ef6dc0`. |
...
- Publication validation now reports 3,275 site assets, 3,272 frontend rows, and 3,275 release assets; the provider-derived expected logical set and site logical set are both 3,272 with zero difference. The MOEX worker-promotion cycle contributed 14 physical/frontend/release assets and changed no non-MOEX bundle; the subsequent WDASEC 2023, 2022, 2021, 2020, 2019, 2015, 2014, 2013, 2012, 2011, 2010, 2009, 2008, 2007, 2006, 2005, 2002, and 2001 cycles contributed 17, 14, 8, 2, 1, 3, 1, 35, 14, 3, 47, 0, 0, 3, 25, 1, 0, and 2 physical/frontend/release assets, respectively; the 2018, 2017, 2004, and 2003 cycles added no new site identity, while the 2016 cycle contributed one new identity.
...
2. Resolve the 18 normalization-gap events and 8 normalized-not-published Hakka events; separately decide whether the evidence-backed WDASEC AD 2002 zero-paper event should be represented by a generic blocked-source exception or remain an unresolved normalization item. The oversized Hakka audio also needs a storage/release decision before it is releasable. The 330 policy-excluded events must remain evidence-backed.
...
4. Reconcile the 770 mixed legacy groups requiring split and document the current physical/logical distinction: 3,273 physical assets, 3,270 logical IDs, with the policy-aware expected logical set equal to the site set.
...
- **Integrity gate:** The strict event-level audit still fails on 18 normalization gaps and 8 normalized-not-published Hakka events; download and parser gaps are 0, and 330 events are explicitly policy-excluded. The official AD 2002 WDASEC event is source-side empty with reproducible evidence, but the repository has no generic blocked-source exception contract. Decide whether to add that audit status or keep it in the repair denominator; the 11 MOEX source/file blockers and remaining gaps also need explicit disposition before deployment is allowed.
